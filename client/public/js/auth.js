document.querySelector('#loginButton1').addEventListener('click', async function (event) {
  ...
  document.querySelectorAll(".link-item a").forEach(link => {
    ...
    link.href = `http://localhost:3000/#${link.getAttribute("data-hash")}`;
    ...
    link.addEventListener("click", function (event) {
      ...
      // Нет необходимости в fetch или изменении innerHTML
      // Браузер автоматически загрузит и отобразит новую страницу
    });
  });
  ...
  function updateHash(newHash) {
    window.location.href = `http://localhost:3000/#${newHash}`;
  }
